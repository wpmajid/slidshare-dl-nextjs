...
// SlideShare's CDN serves WebP for these image URLs even though the path
// ends in ".jpg", and can also fail a request outright under a burst of
// concurrent requests. sharp decodes whatever format actually comes back
// (WebP, JPEG, PNG, ...) and re-encodes it to a real JPEG, which is what the
// rest of the pipeline (pdfkit in particular - it only understands JPEG/PNG)
// needs regardless of what the CDN decided to send. Retrying with a short
// backoff (same as the original Express backend did) covers transient
// failures; if a response truly isn't an image, sharp's own error surfaces.
